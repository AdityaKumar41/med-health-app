  ...
  return useQuery({
    ...
    queryFn: async () => {
      const responose = await axios.get(
        `${process.env.EXPO_PUBLIC_BASE_URL}/v1/patient`,
        {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
...
export const usePatientUpdate = (wallet_address: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationKey: ["patientupdate"],
    mutationFn: async (patient: patientUpdateData) => {
      const responose = await axios({
        method: "put",
        url: `${process.env.EXPO_PUBLIC_BASE_URL}/v1/patient`,
        data: patient,
        headers: {
          Authorization: `Bearer ${wallet_address}`,
          "Content-Type": "application/json",
